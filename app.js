...
function refreshModeMedals() {
  ...
  if (ob) { ob.textContent = m.oboeru ? '🎖️' : ''; ob.style.filter = m.oboeru ? 'hue-rotate(320deg)' : ''; }
...
function medalBadge(dan) {
  ...
  // テストメダルは累積表示
  const items = [
    m.oboeru ? `<span style="${sh}filter:hue-rotate(320deg);">🎖️</span>` : '',
...
function showOboeruClear() {
  ...
  obMedalEl.textContent = '🎖️'; obMedalEl.style.filter = 'hue-rotate(320deg)';
